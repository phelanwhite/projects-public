...
import { blogId } from "@/lib/actions";
import {
  reomveFileToCloundinary,
...
const BlogUpdateById = () => {
  const { id } = useParams();

  useEffect(() => {
    (async () => {
      const resp = await blogId(id as string);

      setFormValue(resp as any);
    })();
  }, [id]);

  const { updateBlogById, getBlogById } = useBlogStore();

  const mutation = useMutation({
    mutationFn: async (data: any) => {
      let thumbnail = formValue.thumbnail;
      if (file) {
        thumbnail = (await uploadFileToCloundinary(file)) as string;
        formValue.thumbnail &&
          (await reomveFileToCloundinary(formValue.thumbnail));
      }

      updateBlogById(id, { ...data, thumbnail });
    },
    onSuccess: () => {
